...
int main(int argc, const char** argv)
{
	...
	for (int i = 0; i < desc->get_nt_headers()->file_header.num_sections; i++)
	{
		...
		for (uint8_t* it = scn_begin; it < (scn_end - 10); it++)
		{
			...
			// 跳过一些条件, 没找到跳转地址所在的区块、跳转地址所在的区块和当前地址是同一个区块、跳转地址所在的区块不可执行
			...
			// 跳过一些条件，文件中跳转地址大于二进制文件大小、跳转后第一条指令不是push、跳转后第二条指令不是call
	...
	//
	// 如果指定了-vms 存放了指定要分析的vm入口容器，则清空存放了vm入口的容器
	if (!target_vms.empty()) desc->virt_routines.clear();

	// 把指定的vm入口存放到 vm入口的容器中
